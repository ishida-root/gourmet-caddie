...
var SALES_FORM_FIELD_MAP={
  name:'sl-name',clientId:'sl-client-id',corp:'sl-corp',planId:'sl-plan',monthlyFee:'sl-amount'
};
...
/* 既存店舗を選んだら内容を自動入力（簡易化：再入力不要にする）
   「商談中」は新規登録時のみ意味を持つ（既契約店舗は更新契約か他商品の商談か
   区別できないため、既存店舗選択時はステータスを触らずチェックボックス自体を隠す） */
function onSlExistingStoreChange(){
  var sel=document.getElementById('sl-existing-store');
  if(!sel)return;
  clearSalesForm();
  var negoRow=document.getElementById('sl-negotiating-row');
  if(!sel.value){
    if(negoRow)negoRow.style.display='';
    return;
  }
  if(negoRow)negoRow.style.display='none';
...
function submitSalesForm(){
  ...
  openSalesCompleteModal(s,sales,plan,negotiating);
}

/* 営業入力の完了画面：SNS通知結果・ヒアリングシート依頼文・原価を表示する */
var _scLastStore=null;
function openSalesCompleteModal(s,sales,plan,negotiating){
  _scLastStore=s;
...
function postStatusBadge(v){var m={draft:'<span class="badge b-gray">下書き</span>',shoot_set:'<span class="badge b-gray">撮影日確定</span>',editing:'<span class="badge b-amber">編集中</span>',delivered:'<span class="badge b-purple">納品済み</span>',scheduled:'<span class="badge b-blue">予約済み</span>',done:'<span class="badge b-green">投稿済み</span>',unbooked:'<span class="badge b-gray">予約未</span>',booked:'<span class="badge b-amber">予約済み</span>',visited:'<span class="badge b-green">来店済み</span>',cancelled:'<span class="badge b-red">キャンセル</span>',pending_review:'<span class="badge b-amber">確認待ち</span>',approved:'<span class="badge b-green">承認済み</span>',date_tbd:'<span class="badge b-amber">🔁 リスケ中（日程未定）</span>'};return m[v]||v;}
