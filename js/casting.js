...
/* ①⊃②⊃③のように上位プランが下位プランの媒体を含んで金額だけ上がっていくケースが多いため、
   複製して媒体チェックはそのまま・プラン名と金額だけ変えれば次のプランを作れるようにする */
function duplicateInfPricePlanRow(idx){
  var src=_pricePlanRows[idx];
  if(!src)return;
  var copy=Object.assign({},src,{id:uid(),platforms:(src.platforms||[]).slice()});
  _pricePlanRows.splice(idx+1,0,copy);
  renderInfPricePlanRows();
}
...
function infOutreachEmailTemplate(inf){
  ...
  return name+'\n\n'
    ...
    +'・ご来店から投稿までのおおよその日数\n\n'
    +'なお、ご来店時のお食事代は飲食店様ではなく弊社（ルート株式会社）からのお支払いとなるため、当日は一旦ご自身でお支払いいただき、後日PR費用と合わせて立替精算という形でお戻しいたします。\n\n'
    +'現時点は条件確認の段階であり、確定のご依頼ではございません。飲食店様からの正式なご発注が決まり次第、改めて具体的な案件のご相談をさせていただければと存じます。\n\n'
...
function infPrOfferTemplate(inf,store,plan){
  ...
  return name+'\n\n'
    ...
    +'・ご来店時のお食事代は、弊社からのご依頼のため無償ではなく、当日一旦ご自身でお支払いいただいた分を後日PR費用と合わせて立替精算という形でお戻しいたします（お一人5,000円・税込5,500円が上限）\n'
